...
exports.getMRR = function(cb){
    ...
    material_receive_form.SUB_TOTAL , material_receive_form.DATE_DELIVERED FROM material_receive_form 
...
exports.addMRR = function(prj_id, mrs_id, quantity , unit_cost ,sub_total , date_delivered ,  cb){
    ...
    con.query(sql,[prj_id , mrs_id , quantity , unit_cost , sub_total , date_delivered],function(err,result){
        if(err){
            cb({status: 'failed' , error:err})
        }else{
            cb(null,result)
